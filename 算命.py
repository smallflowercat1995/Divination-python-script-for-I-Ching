...
请将代码分段，每段字数限制1800字，先发送第一段代码，然后，在我每次回复`继续`的时候，再继续发送下一段 
要求每段发送格式如下
...
发送完代码之后，请将`历史文件`内容也分段，每段字数限制1800字，先发送第一段代码，然后，在我每次回复`继续`的时候，再继续发送下一段 
要求每段发送格式如下
...
发送完历史文件内容之后，请将存储`爻题、爻辞和爻辞白话解释的 yao.json 文件`内容也分段，每段字数限制1800字，先发送第一段代码，然后，在我每次回复`继续`的时候，再继续发送下一段 
要求每段发送格式如下

    `爻题、爻辞和爻辞白话解释的json文件第n段` 
    ```
    json格式内容
    ```

发送完根据卦象图查找周易卦象名对照表文件内容之后，请将存储`卦名、卦辞、卦辞白话解释的 hexagram.json 文件`内容也分段，每段字数限制1800字，先发送第一段代码，然后，在我每次回复`继续`的时候，再继续发送下一段 
要求每段发送格式如下

    `卦名、卦辞、卦辞白话解释的json文件第n段` 
    ```
    json格式内容
    ```
